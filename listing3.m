...
%%% Inizialization %%%
%%%%%%%%%%%%%%%%%%%%%%%%%
freq_freerunning = fref*N; % free running frequency of the controlled oscillator
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
swichTDCdtr = 0; % set swichTDCdtr = 0 in order to switch off the dither
dither = swichTDCdtr*(1/3)*randn(1,length(time_array)); %three sigma of this signal is equal to 1
for n = 2:1:length(time_array)
  ...
  InputSigmaDelta = tin(n)/resTDC;
  outTDC(n) = resTDC * floor( InputSigmaDelta + eq(n -1)  );
  eq(n) = ((InputSigmaDelta + eq(n-1)) - floor(  InputSigmaDelta + eq(n -1) ));
...
[PSDeq,feq] = pwelch( resTDC*eq, window_length,[],[],1/Ts,'twosided');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
PSDinputDCO_TDCquantComponent = (abs(HclRef)./abs(Hint*kv)).^2 * ( VarTDCquant) .*(2-2*cos(w)) ;
...
set(h(3), 'LineWidth', 2)
set(h(4), 'LineWidth', 3)
axis([1e3 0.5e7 -150 -104])
legend('Matlab Model','Analytical Predictions', 'DCOnoise', 'TDC noise')
